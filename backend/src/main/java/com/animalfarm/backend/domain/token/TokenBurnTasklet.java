	...
	public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
		...
		try {
			...
			if (refundOriginList != null && !refundOriginList.isEmpty()) {
				List<Long> walletIds = refundOriginList.stream()
					.map(RefundDTO::getWalletId)
					.collect(Collectors.toList());

				// walletId 리스트로 한 번에 userId, shId를 조인해서 가져옴
				List<RefundDTO> dbInfoList = refundRepository.selectRefundInfoByWalletIds(walletIds, projectId,
					tokenId);
