...
public class SubscriptionService {
	...
	public void selectAllocationInfo(Long inprogressProjectId) {
		...
		if (status.getMinAmount().compareTo(standardAmount) >= 0) {
			...
			lastPrevHash = processCase3(dto, investors, TOKEN_SCALE, requestList, newTokenList, projectId, tokenId,
			...
			if (investor != null) {
				...
				if (refundAmount.compareTo(BigDecimal.ZERO) > 0) {
					RefundDTO refund = RefundDTO.builder()
						...
						.projectId(projectId) // 상단에서 선언한
						.shId(investor.getShId())
						.uclId(res.getWalletId()) // 증권사 지갑 ID
						.amount(refundAmount)
						.refundType("PARTIAL") // 일부 배정 후 남은 금액이므로 PARTIAL
	...
	*/
	...
	private String processCase1(AllocationTokenDTO dto,
		...
		for (InvestorDTO sendData : investors) {
			...
			String newHash = HashManager.createHash(lastPrevHash, tokenId, resultTokenCount);
	...
	private String processCase2(AllocationTokenDTO dto,
	...
	private String processCase3(AllocationTokenDTO dto,
		List<InvestorDTO> investors,
		int TOKEN_SCALE,
		List<AllocationRequestDTO> requestList,
		List<TokenLedgerDTO> newTokenList,
