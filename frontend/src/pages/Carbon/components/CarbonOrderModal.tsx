...
export default function CarbonOrderModal({ 
  ...
  const displayUnitPrice = quoteData.unitPrice > 0 ? quoteData.unitPrice : unitPrice;
  const displayMaxQty = quoteData.maxQty > 0 ? quoteData.maxQty : maxQty;
  const displayTotal = quoteData.totalAmount > 0 ? quoteData.totalAmount : (displayUnitPrice * currentQty);
  const displaySupply = quoteData.supplyAmount > 0 ? quoteData.supplyAmount : Math.round(displayTotal / 1.1);
  const displayVat = quoteData.vatAmount > 0 ? quoteData.vatAmount : (displayTotal - displaySupply);
    ...
    const fetchQuote = async () => {
      try {
        ...
        if (p) {
          ...
          const max = Math.max(0, Math.min(userMax || remain, remain || userMax));
  ...
  const handleClose = () => {
    ...
    setIsAgreed(false);
    // 모달 닫을 때 이전 값 찌꺼기 청소
    setQuoteData({ maxQty: 0, unitPrice: 0, supplyAmount: 0, vatAmount: 0, totalAmount: 0 });
    onClose();          
  };
  ...
  const handleSubmitOrder = async () => {
    if (!window.IMP) {
      alert("결제 모듈(PortOne)을 불러오지 못했습니다. 새로고침 후 다시 시도해주세요.");
      return;
    }
    ...
    IMP.request_pay({
      ...
      if (rsp.success) {
        try {
          ...
          await axios.post(`${baseURL}/api/carbon/orders/complete`, {
            impUid: rsp.imp_uid,
          ...
          console.error(error);
          alert("결제는 완료됐지만 서버 검증 처리에 실패했습니다. 관리자에게 문의하세요.");
        } finally {
    ...
    <div className="fixed inset-0 flex items-center justify-center overflow-y-auto" style={{ zIndex: 9999 }}>
      ...
      <div className="relative z-10 my-4 flex max-h-[calc(100dvh-32px)] w-[calc(100%-32px)] max-w-[440px] flex-col overflow-y-auto rounded-(--radius-lg) bg-white font-main shadow-(--shadow-std)">
        ...
        <div className="flex items-center justify-between px-[24px] pt-[24px] pb-[16px]">
          <h3 className="m-0 font-subtitle-01 text-(--color-gray-900)">주문 신청하기</h3>
          <button type="button" onClick={handleClose} className="flex items-center justify-center w-[32px] h-[32px] border-none bg-transparent text-(--color-gray-500) text-[22px] cursor-pointer rounded-(--radius-s) hover:bg-(--color-gray-50) hover:text-(--color-gray-800)">
            ×
          </button>
        </div>
        ...
        <div className="px-[24px] pb-[24px] flex flex-col gap-[16px]">
          ...
          <div className="flex items-center gap-[14px] p-[14px] bg-(--color-gray-50) rounded-(--radius-m)">
            ...
            <div className="flex flex-col gap-[4px]">
              <div className="font-body-04 text-(--color-gray-900) leading-[1.3]">{productName}</div>
              <div className="font-button-02 text-(--color-green-600)">단가 {displayUnitPrice.toLocaleString()}원 (VAT 별도)</div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-[8px]">
            <div className="flex justify-between items-center">
              <span className="font-caption-01 text-(--color-gray-500)">최대 구매 가능 수량</span>
              <span className="font-caption-02 text-(--color-green-600)">{displayMaxQty.toLocaleString()} tCO2e</span>
            </div>
          </div>
          ...
          <div className="bg-(--color-gray-50) rounded-(--radius-m) p-[16px] flex flex-col gap-[8px]">
            <div className="flex justify-between items-center">
              <span className="font-caption-01 text-(--color-gray-600)">총 공급가액</span>
              <span className="font-caption-02 text-(--color-gray-900)">{displaySupply.toLocaleString()}원</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="font-caption-01 text-(--color-gray-600)">부가세 (VAT 10%)</span>
              <span className="font-caption-02 text-(--color-gray-900)">{displayVat.toLocaleString()}원</span>
            </div>
            <div className="h-px bg-(--color-gray-200) my-[4px]" />
            <div className="flex justify-between items-center">
              <span className="font-caption-02 text-(--color-gray-900)">총 결제 금액 (VAT포함)</span>
              <span className="font-subtitle-01 text-(--color-green-600)">{displayTotal.toLocaleString()}원</span>
            </div>
          </div>
          ...
          <label className="flex gap-[8px] items-start cursor-pointer">
            ...
            <span className="font-caption-01 text-(--color-gray-500) leading-[1.4]">
              본 주문 건의 <a href="/policy" target="_blank" className="text-(--color-gray-600) underline underline-offset-2 font-caption-02">이용약관</a> 및 <a href="/policy" target="_blank" className="text-(--color-gray-600) underline underline-offset-2 font-caption-02">탄소거래규정</a>에 동의하며, 자산 매입 확약에 따른 결제를 진행합니다.
            </span>
          </label>
