...
export const myPageApi = {
  ...
  getHoldings: (page = 1) =>
    apiClient.get<HoldingDTO[], HoldingDTO[]>(`/api/mypage/holdings?page=${page}`),

  linkAccount: () =>
    apiClient.get<number, number>("/api/mypage/account/link"),
  ...
  getProjects: ({
    ...
    apiClient.get<PagedResponseDTO<MyPageProjectDTO>, PagedResponseDTO<MyPageProjectDTO>>(
      `/api/mypage/projects?type=${type}&status=${status}&page=${page}&size=${size}`,
    ),

  // 탄소 배출권 구매 내역
  getCarbonHistory: () =>
    apiClient.get<CarbonHistoryDTO[], CarbonHistoryDTO[]>("/api/mypage/carbon-history"),
