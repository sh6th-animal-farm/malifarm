...
export const toCardModel = (dto: ProjectDTO | Project): Project => {
  ...
  const safeStatus: ProjectStatus =
    status === "SUBSCRIPTION" || status === "ANNOUNCEMENT" || status === "INPROGRESS"
      ? status
      : "SUBSCRIPTION";
  ...
  const projectName = (dto as ProjectDTO).projectName ?? (dto as Project).title ?? "";
  const projectRound = (dto as ProjectDTO).projectRound;
  const title = projectRound ? `${projectName} ${projectRound}회차` : projectName;
  ...
  const dDayTarget = isSubscription
    ? (dto as ProjectDTO).subscriptionEndDate ?? (dto as Project).countdownTarget
    : isAnnouncement
      ? (dto as ProjectDTO).announcementEndDate ?? (dto as Project).countdownTarget
      : null;
  ...
  return {
    ...
    upperDate,
    lowerDate,
    percent: (dto as ProjectDTO).subscriptionRate ?? (dto as Project).percent ?? 0,
